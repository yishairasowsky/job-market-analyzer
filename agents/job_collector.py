...
def collect_jobs(on_progress=None):
    def progress(msg):
        print(msg)
        if on_progress:
            on_progress(msg)

    progress("Agent 1 (Job Collector): Fetching job postings...")
    ...
    summary = f"  Found {len(relevant)} new job postings ({source_summary})."
    progress(summary)
    return relevant
...
def _fetch_arbeitnow(progress=None):
    try:
        response = requests.get(ARBEITNOW_URL, timeout=15)
        response.raise_for_status()
        jobs = response.json().get("data", [])
    except Exception as e:
        msg = f"  Warning: Arbeitnow unavailable — {e}"
        print(msg)
        if progress:
            progress(msg)
        return []
...
def _fetch_remotive(progress=None):
    results = []
    for category in REMOTIVE_CATEGORIES:
        try:
            ...
            msg = f"  Warning: Remotive ({category}) unavailable — {e}"
            print(msg)
            if progress:
                progress(msg)
            continue
...
def _fetch_remoteok(progress=None):
    try:
        response = requests.get(
            REMOTEOK_URL,
        ...
        msg = f"  Warning: RemoteOK unavailable — {e}"
        print(msg)
        if progress:
            progress(msg)
        return []
...
def _fetch_hn_hiring(progress=None):
    ...
    for thread in threads:
        ...
        if progress:
            progress(f"  Scanning {thread_label}...")
        try:
            ...
            msg = f"  Warning: Could not fetch HN comments for {thread_label} — {e}"
            print(msg)
            if progress:
                progress(msg)
            continue
